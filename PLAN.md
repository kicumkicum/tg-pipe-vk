...
- [ ] Подготовить локальную разработку:
  - [ ] Установить Vercel CLI и завести `vercel dev`
  - [ ] Настроить локальные переменные окружения (например через `.env.local`, без коммита секретов)
- [ ] Определить и задокументировать публичные эндпоинты:
  - [ ] `POST /api/vk`
  - [ ] `POST /api/telegram`
- [ ] (опционально) Добавить `vercel.json`, если нужно явно зафиксировать runtime/роутинг/лимиты
